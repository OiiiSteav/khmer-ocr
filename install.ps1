...
try {
    ...
    Write-Host "Generating custom Windows icon (logo_v5.ico)..." -ForegroundColor Yellow
    Start-Process ".\venv\Scripts\python.exe" -ArgumentList "make_png_ico.py" -Wait -NoNewWindow
} catch {
...
try {
    ...
    $Shortcut.IconLocation = "$TargetDir\logo_v5.ico" # Assign custom branded icon
